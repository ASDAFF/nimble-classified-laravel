(function ($, w, undefined) {
    w.footable = {
        options: {
            ...
            _validate: function (plugin) {
                ...
                return true;
            }, registered: [], register: function (plugin, options) {
    ...
    function Footable(t, o, id) {
        ...
        var opt = ft.options, cls = opt.classes, evt = opt.events, trg = opt.triggers, indexOffset = 0;
        ft.timers = {
            resize: new Timer(), register: function (name) {
        ...
        ft.addRowToggle = function () {
            ...
            var $table = $(ft.table), hasToggleColumn = false;
        ...
        ft.resize = function () {
            ...
            if (!pinfo || (pinfo && pinfo.width && pinfo.width !== info.width)) {
                ...
                var breakpointName = (current === null ? 'default' : current['name']), hasBreakpointFired = ft.hasBreakpointColumn(breakpointName), previousBreakpoint = $table.data('breakpoint');
        ...
        ft.redraw = function () {
            ...
            ft.setColumnClasses();
            var $table = $(ft.table), breakpointName = $table.data('breakpoint'), hasBreakpointFired = ft.hasBreakpointColumn(breakpointName);
            $table.find('> tbody > tr:not(.' + cls.detail + ')').data('detail_created', false).end().find('> thead > tr:last-child > th').each(function () {
                ...
                if (breakpointName !== '') {
                    if (data.hide[breakpointName] === false)$column.addClass('footable-visible').show(); else $column.removeClass('footable-visible').hide();
                }
            ...
            $table.find('> tbody > tr.' + cls.detailShow + ':visible').each(function () {
                ...
                if ($next.hasClass(cls.detail)) {
                    if (!hasBreakpointFired)$next.hide(); else $next.show();
                }
            });
        ...
        ft.toggleDetail = function (row) {
            var $row = (row.jquery) ? row : $(row), $next = $row.next();
