		jQuery(document).ready(function($){
			$('.preset-list li a').on('click',function(event){
				...
				var color = $(this).data('color'),
					url = base_url+'/assets/css/presets/'+color+'.css';
				...
				$.post(base_url+'/theme-css', {css:color}, function (data) {
					if(data.msg == 1){
						$('.style-chooser h4').text('Style changed!');
					}else{
                        $('.style-chooser h4').text('Some error!');
					}
                });
			});
